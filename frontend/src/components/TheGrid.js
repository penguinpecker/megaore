...
const GRID_ABI = [
  ...
  { name: "roundDuration", type: "function", stateMutability: "view",
    inputs: [], outputs: [{ name: "", type: "uint256" }] },
  { name: "gridPerRound", type: "function", stateMutability: "view",
    inputs: [], outputs: [{ name: "", type: "uint256" }] },
...
export default function TheGrid() {
  ...
  const pollState = useCallback(async () => {
    ...
    try {
      ...
      // 2. Get round data — field order: [0]start [1]end [2]pot [3]totalPlayers [4]winningCell [5]resolved [6]extra
      try {
      ...
      try {
        if (!resolvedRef.current && rNum > 0) {
          ...
          if (now - lastAttempt > 2000 && roundEnd > 0 && nowSec >= roundEnd) {
            ...
            if (RESOLVER_URL) {
              console.log("[The Grid] Round ended, calling resolver directly...");
              fetch(RESOLVER_URL, { method: "POST" })
                .then(r => r.json())
                .then(d => {
                  console.log("[The Grid] Resolver response:", d);
                  if ((d.action === "resolved" || d.action === "fulfill") && d.tx) resolverTxHash.current = d.tx;
                })
                .catch(err => console.error("[The Grid] Resolver fetch error:", err));
            }
          }
        }
      } catch (e) {
  ...
  return (
    <div style={S.root}>
      ...
      <header style={S.header} className="grid-header">
        ...
        <div style={{ ...S.hRight, gap: 10 }}>
          ...
          {authenticated && (
            <>
              <span style={S.hStat} className="grid-header-stat">
                ● {fmt(gridBalance, 2)} <b style={{ color: "#ff6633" }}>GRID</b>
              </span>
              <span style={S.hStat} className="grid-header-stat">
          ...
          {authenticated && (
            <span className="grid-mobile-balances" style={{
              ...
              <span style={{ color: "#ff6633" }}>{fmt(gridBalance, 2)} <b>GRID</b></span>
            </span>
          )}
          {!authenticated ? (
      ...
      <div style={S.main} className="grid-main">
        ...
        <div style={S.sidebar} className={`grid-sidebar ${mobileMenu ? "open" : ""}`}>
          ...
          <Panel title="SECTOR ANALYSIS" live>
            ...
            <Row label="MOTHERLODE" value="0.4 GRID" />
      ...
      <footer style={S.footer}>
        ...
        <span style={{ fontSize: 11, color: "#4a5a6e", letterSpacing: 1 }}>5×5GRID.COM ◆ MEGAETH ◆ CHAIN 4326</span>
      </footer>
