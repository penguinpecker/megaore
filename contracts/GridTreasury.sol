...
/**
 * @title GridTreasury
 * @notice Receives protocol fees from TheGrid and splits:
 *         - 40% → Team wallet
 *         - 30% → Buyback GRID from DEX + burn
 *         - 30% → NFT staker rewards
 */
contract GridTreasury is Ownable, ReentrancyGuard {
    ...
    event BuybackExecuted(uint256 ethSpent, uint256 gridBurned);
    ...
    constructor(address _gridToken, address _teamWallet) Ownable(msg.sender) {
        if (_gridToken == address(0) || _teamWallet == address(0)) revert ZeroAddress();
    ...
    function burnGrid(uint256 amount) external onlyOwner {
        gridToken.burn(amount);
        totalBurned += amount;
