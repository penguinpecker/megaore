...
contract OreGrid is Ownable, ReentrancyGuard {
    ...
    constructor(
        address _oreToken, address _nftStaking, address _treasury,
        uint256 _depositAmount, uint256 _roundDuration, uint256 _orePerRound,
        uint256 _protocolFeeBps, uint256 _motherlodePerRound, uint256 _nftBoostBps, uint256 _resolverReward
    ) Ownable(msg.sender) {
    ...
    function resolveRound() external nonReentrant {
        ...
        if (round.totalPlayers == 0) { _startNewRound(); return; }

        uint8 winningCell = Randomness.getWinningCell(roundId, round.totalDeposits, round.totalPlayers);
        ...
        if (fee > 0) { treasury.depositFee{value: fee}(roundId); totalFeesCollected += fee; }
        motherlodePool += motherlodePerRound;

        bool motherlodeTriggered = false;
        uint256 motherlodePayout = 0;
        if (winnersCount > 0) {
            uint256 motherlodeRng = uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), roundId, "motherlode"))) % MOTHERLODE_ODDS;
            if (motherlodeRng == 0) { motherlodeTriggered = true; motherlodePayout = motherlodePool; motherlodePool = 0; totalMotherlodesPaid++; }
        }

        if (winnersCount == 0) {
            if (distributablePot > 0) { treasury.depositFee{value: distributablePot}(roundId); }
            ...
            for (uint256 i = 0; i < winnersCount; i++) {
                ...
                if (potPerWinner > 0) { (bool ok, ) = winner.call{value: potPerWinner}(""); if (!ok) revert TransferFailed(); }
                uint256 oreReward = baseOre;
                if (nftStaking.isStaker(winner)) { oreReward += (baseOre * nftBoostBps) / 10000; }
                if (oreReward > 0) { oreToken.mint(winner, oreReward); totalOREMinted += oreReward; }
                if (motherlodeTriggered && motherlodePayout > 0) { uint256 motherShare = motherlodePayout / winnersCount; oreToken.mint(winner, motherShare); totalOREMinted += motherShare; }
            }
            uint256 dust = distributablePot - (potPerWinner * winnersCount);
            if (dust > 0) { treasury.depositFee{value: dust}(roundId); }
            emit RoundResolved(roundId, winningCell, winnersCount, potPerWinner, baseOre, motherlodeTriggered);
            if (motherlodeTriggered) { emit MotherlodeTriggered(roundId, motherlodePayout, winnersCount); }
        }

        if (resolverReward > 0) { oreToken.mint(msg.sender, resolverReward); totalOREMinted += resolverReward; }
        totalRoundsPlayed++;
    ...
    function getCellPlayers(uint256 roundId, uint8 cell) external view returns (address[] memory) { return cellPlayers[roundId][cell]; }

    function getCellCounts() external view returns (uint16[25] memory counts) {
        for (uint8 i = 0; i < GRID_SIZE; i++) { counts[i] = uint16(cellPlayers[currentRoundId][i].length); }
    }

    function canResolve() external view returns (bool) { Round memory r = rounds[currentRoundId]; return block.timestamp >= r.endTime && !r.resolved; }

    function getRoundResult(uint256 roundId) external view returns (uint8 winningCell, uint256 winnersCount, bool resolved) {
    ...
    function _startNewRound() internal {
        ...
        rounds[currentRoundId] = Round({ startTime: start, endTime: end, totalDeposits: 0, totalPlayers: 0, winningCell: 0, resolved: false });
        emit RoundStarted(currentRoundId, start, end);
    }

    function emergencyWithdraw() external onlyOwner { (bool ok, ) = owner().call{value: address(this).balance}(""); if (!ok) revert TransferFailed(); }
}
